...
# Crear el modelo con MobileNetV2 y descongelar capas superiores
base_model = MobileNetV2(weights='imagenet', include_top=False, input_tensor=Input(shape=INPUT_SHAPE))
for layer in base_model.layers[-50:]:  # Descongelar las últimas 50 capas
...
model_checkpoint = ModelCheckpoint('best_model_v4.h5', monitor='val_loss', save_best_only=True, verbose=1)
...
history = model.fit(
    ...
    epochs=EPOCHS,
    callbacks=[CustomProgressCallback(), early_stopping, model_checkpoint, reduce_lr]
)
